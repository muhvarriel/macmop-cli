            ...
            for (name, path) in &ctx.paths.cloud_dirs {
                    ...
                    .enumerate()
                {
                    if visited_entries >= 10_000 {
                        scan_limited = true;
                        warnings.push("Scan was bounded; sampled values may not represent the full cloud folder.".to_string());
                        break;
                    }
                    match entry {
                        Ok(entry) => {
                        ...
                        Err(error) => {
                            ...
                            warnings
                                .push(format!("could not scan {}: {error}", path_err.display()));
                        }
                    }
                }

                total_sampled_size_bytes =
                    total_sampled_size_bytes.saturating_add(sampled_size_bytes);
                ...
                findings.push(json!({
                    "id": format!("cloud_{}_{}", name.to_lowercase().replace(' ', "_"), crate::core::unix_now()),
                    "module": "cloud",
                    "category": "sync",
                    "path": path,
                    "size_bytes": sampled_size_bytes,
                    "risk": "low",
                    "confidence": 1.0,
                    "action": "report_only",
                    "reason": format!("Cloud sync path detected: {} (contains warning)", name),
                    "requires_sudo": false,
                }));
            }
