...
pub mod startup {
    ...
    pub fn scan_startup_dirs(
        startup_dirs: &[(PathBuf, String)],
    ) -> (Vec<StartupItem>, Vec<String>) {
            ...
            if shell_interpreters.contains(&launcher_name) {
                ...
                for arg in &item.program_arguments {
                    ...
                    if arg_lower.contains("curl") && arg_lower.contains("sh") {
                        evidence.push(
                            "Contains nested network download/execution (curl | sh)".to_string(),
                        );
                        combined_high = true;
                    }
...
pub mod privacy {
    ...
    pub fn run(
        ...
        match args.command {
            ...
            crate::cli::PrivacyCommand::Recent => {
                scan(ctx, "privacy recent", false, true, false, true)
            }
        }
    }
            ...
            match task.id.as_str() {
                "flush_dns" => {
                    ...
                    task.reason = if task.available {
                        "Available on macOS; alpha.6 reports only and does not flush DNS."
                            .to_string()
                    } else {
                ...
                "thin_time_machine_snapshots" => {
                    ...
                    task.reason = if task.available {
                        "tmutil is discoverable; future execution would require explicit sudo handling.".to_string()
                    } else {
